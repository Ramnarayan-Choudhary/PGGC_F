...
def main():
    ...
    for i, item in enumerate(data):
        ...
        # Reconstruct Prompt
        # Ensure we convert tensors to python integers for JSON serialization
        full_prompt_tokens = []
        for tp in updated_token_maps:
            tid = tp['token_id']
            if torch.is_tensor(tid):
                tid = tid.item()
            full_prompt_tokens.append(int(tid))
        ...
        # 4. Save the prompt for the Target Model (Evaluation Phase)
        item["final_prompt"] = tokenizer.decode(full_prompt_tokens, skip_special_tokens=True)
        # Also save the raw tokens just in case
        item["final_tokens"] = full_prompt_tokens
        
        print(f"Sanitized Prompt generated. Saved for evaluation.")
    ...
    print(f"Done. Generated {processed_count}/{args.total_to_do or len(data)} sanitized prompts. Saved to {args.output_file}")
    print(f"To evaluate accuracy, run: python scripts/evaluate_disguise.py --input_file {args.output_file} --model <TARGET_MODEL_ID>")
